import unittest
import os
from nb2opr.exporters import add_code_to_operator, create_operator
...
from nb2opr.utility_objects import PortObject, ConnectionObject, OperatorObject, PortKind, ContentType
import sapdi as di
import warnings
import urllib3
import logging
...
class TestProcessor(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.s2 = {
            ...
            'src_port_name': 'metrics1',
            'tgt_opr_name': 'submitmetrics1',
            'tgt_port_name': 'response1',
    ...
    def test_s3_add_port_to_operator_success(self):
        ...
        port = PortObject(name=self.s3['port_name'], kind=PortKind.OUTPUT, content_type=ContentType.STRING)
        port_added = add_port_to_operator(opr_name=self.s3['operator_name'], port=port, target=self.s3['target'])
        self.assertTrue(port_added)
    ...
    def test_s3_add_port_to_operator_invalid_operator(self):
        ...
        port = PortObject(name=self.s3['port_name'], kind=PortKind.OUTPUT, content_type=ContentType.STRING)
        self.assertRaises(Exception, lambda: add_port_to_operator(opr_name=self.s3['invalid_opr_name'], port=port))


    def test_s4_create_operator_success(self):
        opr = OperatorObject(name="com.sap.system.python3Operator")
        op = create_operator(opr)
        self.assertIsInstance(op, di.internal.modeler.operator.Operator)

    def test_s4_create_operator_invalid_params(self):
        self.assertRaises(Exception, lambda: create_operator(opr="com.sap.system.pthon3operator"))

    def test_s5_get_pipelines(self):
